...
- `src/content/default.md` - Default landing content for right column (supports hero image with frontmatter)

**Link Formats in Markdown**:
- `[Label](@contentKey)` - Internal link, loads content from `src/content/` (e.g., `@assignments/week-01-a`)
  - Links automatically get emoji prefixes based on folder: 🧰 workshops, 🧩 assignments, 📙 readings, 🚀 projects
- `[Label](https://...)` - External links automatically open in new tab
  - Links starting with "Lecture" get 🧑🏻‍🏫 emoji prefix
...
- `public/sketches/` - p5.js sketch files

Note: Use URL-encoded paths or rename files to avoid spaces (e.g., `my-document.pdf` not `my document.pdf`).

**p5.js Sketch Embedding**:
- Use `<sketch src="sketch-name.js"></sketch>` in markdown to embed interactive p5.js sketches
- Sketches are loaded from `public/sketches/` directory
- Each sketch includes a replay button to restart the animation
- Active p5 instances are tracked and cleaned up when switching content
